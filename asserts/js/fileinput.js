...
(function ($) {
    ...
    var isIE = function (ver) {
        ...
        tCaption = '<div tabindex="-1" class="form-control file-caption {class}" style="border:none;font-size: 3vw;line-height: 1;">\n' +
        ...
        tModal = '<div id="{id}" class="modal fade">\n' +
            ...
            '        <h3 class="modal-title">详细 <small>{title}</small></h3>\n' +
            '      </div>\n' +
    ...
    $.fn.fileinput.locales.en = {
        ...
        removeTitle: '清除选择文件',
        cancelLabel: '取消',
        cancelTitle: '取消正在进行的上传',
        uploadLabel: '上传',
        uploadTitle: '选择上传文件',
        msgSizeTooLarge: '文件："{name}"大小超出5MB,请重新上传!',
        msgFilesTooLess: '你必须选择 <b>{n}</b> {files} 再点击该按钮. 请重试!',
        msgFilesTooMany: '您选择的图片多于20张,请删除至20张重试!',
        msgFileNotFound: '文件 "{name}" 不存在!',
        msgFileSecured: '您的安全策略阻止上传 "{name}"。',
        msgFileNotReadable: '文件： "{name}" 是不可读的。',
        msgFilePreviewAborted: '文件："{name}"预览中止。',
        msgFilePreviewError: '在读取："{name}"时发生错误。',
        msgInvalidFileType: ' "{name}"格式不支持，只有 "{types}" 格式文件支持。',
        msgInvalidFileExtension: '"{name}"扩展名无效，只有 "{extensions}" 扩展名有效。',
        msgValidationError: '文件上传出现错误',
        msgLoading: '加载 {index} 的 {files} &hellip;',
        msgProgress: '加载 {index} 的 {files} - {name} - {percent}% 完成.',
        msgSelected: '{n} 个文件选择',
        msgFoldersNotAllowed: 'Drag & drop files only! {n} folder(s) dropped were skipped.',
        dropZoneTitle: '您上传的图片将在这里预览 &hellip;'
    };
